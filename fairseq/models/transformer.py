...
class TransformerModel(FairseqEncoderDecoderModel):
    ...
    def add_args(parser):
        ...
        parser.add_argument('--dont-use-layernorm', action='store_true', default=False)
        ################### Adding arguments for CMLMC ######
...
class TransformerEncoder(FairseqEncoder):
    ...
    def __init__(self, args, dictionary, embed_tokens):
        ...
        if self.concatPE:
            self.PEfc = nn.Linear(2*embed_dim, embed_dim)
    ...
    def forward_embedding(self, src_tokens):
        ...
        if self.embed_positions is not None:
            if self.concatPE:
                x = self.PEfc(torch.cat((embed, self.embed_positions(src_tokens)), -1))
            else:
...
class TransformerDecoder(FairseqIncrementalDecoder):
    ...
    def build_decoder_layer(self, args, no_encoder_attn=False, layernum=0):
        return TransformerDecoderLayer(args, no_encoder_attn, layernum=layernum)
