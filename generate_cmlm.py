...
def generate(strategy, encoder_input, models, tgt_dict, length_beam_size, gold_target_len):
    ...
    # print('in generate_cmlm, bsz: ', bsz)
    encoder_out = model.encoder(**encoder_input)
    # print('encoder_out.encoder_out size: ', encoder_out.encoder_out.size())
    predicted_length = model.decoder.forward_length(normalize=True, encoder_out=encoder_out)
    # predicted_length = model.decoder.forward_length_prediction(
    #     model.decoder.forward_length(normalize=True, encoder_out=encoder_out), encoder_out)
    # print('length_beam_size: ', length_beam_size)
    # print('gold_target_len: ', gold_target_len)
    # print('predicted length size: ', predicted_length.size())
    # print(predicted_length)
    beam = predict_length_beam(gold_target_len, predicted_length, length_beam_size)
    ...
    #
    # modify to fit the main branch fairseq:
    # - dictionary doesn't have methods such as mask() or pad() implemented, need to directly return indices
    # - it uses the unk index to represent masked ones...
    #   -- in cmlm_transformer.py line 73
    #   -- output_masks = output_tokens.eq(self.unk)
    # - actually, every other methods (pad(), eos(), bos(), unk()) are all implemented. Since mask is treated the same
    # as unk, just return the unk() instead
    tgt_tokens = src_tokens.new(bsz, length_beam_size, max_len).fill_(tgt_dict.unk())
    # print('at creation, tgt_tokens size: ', tgt_tokens.size())
    ...
    # #############################################################
    # print('before duplicate, encoder_out.encoder_out size: ', encoder_out.encoder_out.size())
    # duplicate_encoder_out(encoder_out, bsz, length_beam_size)
    encoder_out = encoder_out._replace(
        encoder_out=encoder_out.encoder_out.unsqueeze(2).repeat(1, 1, length_beam_size, 1).view(-1,
                                                                                                bsz * length_beam_size,
                                                                                                encoder_out.encoder_out.size(
                                                                                                    -1)))
    if encoder_out.encoder_padding_mask is not None:
        encoder_out = encoder_out._replace(
            encoder_padding_mask=encoder_out.encoder_padding_mask.unsqueeze(1).repeat(1, length_beam_size, 1).view(
                bsz * length_beam_size, -1))

    # print('after duplicate, encoder_out.encoder_out size: ', encoder_out.encoder_out.size())
